"""Live API tests for streaming functionality.

Tests real WebSocket connections and data streaming with TradingView.
"""
...
@pytest.mark.live
class TestLiveStreamingCandles:
    """Test historical candle fetching via WebSocket."""
    ...
    def test_live_get_candles_multiple_timeframes(self) -> None:
        ...
        for tf in timeframes:
            streamer = Streamer()
            result = streamer.get_candles(
                exchange="BINANCE", symbol="BTCUSDT", timeframe=tf, numb_candles=3
            )
    ...
    @pytest.mark.skip(
        reason="Requires JWT token - indicator fetching needs authentication"
    )
    ...
    def test_live_get_candles_crypto(self) -> None:
        ...
        streamer = Streamer()
        result = streamer.get_candles(
            exchange="BINANCE", symbol="BTCUSDT", timeframe="5m", numb_candles=5
        )
        assert result["status"] == STATUS_SUCCESS
        assert len(result["data"]["ohlcv"]) >= 1
    ...
    def test_live_get_candles_invalid_symbol_handling(self) -> None:
    ...
    def test_live_get_candles_large_candle_request(self) -> None:
...
@pytest.mark.live
class TestLiveStreamingRealtimePrice:
...
class TestLiveStreamingCombinations:
    ...
    def test_various_crypto_symbols(self) -> None:
        ...
        for symbol in symbols:
            streamer = Streamer()
            result = streamer.get_candles(
                exchange="BINANCE", symbol=symbol, timeframe="5m", numb_candles=3
            )
