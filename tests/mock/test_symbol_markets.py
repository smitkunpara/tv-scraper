...
class TestMockSymbolMarketsBasic:
    ...
    def test_mock_aapl_global_success(self, scraper: SymbolMarkets) -> None:
        ...
        with patch.object(scraper, "_request") as mock_req:
            ...
            result = scraper.get_symbol_markets(symbol="AAPL", scanner="global")
            assert result["status"] == STATUS_SUCCESS
            assert isinstance(result["data"], list)

    def test_mock_btcusd_crypto_success(self, scraper: SymbolMarkets) -> None:
        ...
        with patch.object(scraper, "_request") as mock_req:
            ...
            result = scraper.get_symbol_markets(symbol="BTCUSD", scanner="crypto")
            assert "status" in result

    def test_mock_eurusd_forex_success(self, scraper: SymbolMarkets) -> None:
        ...
        with patch.object(scraper, "_request") as mock_req:
            mock_req.return_value = ({"data": mock_data, "totalCount": 1}, None)
            result = scraper.get_symbol_markets(symbol="EURUSD", scanner="forex")
            assert "status" in result
...
class TestMockSymbolMarketsFields:
    ...
    def test_mock_default_fields(self, scraper: SymbolMarkets) -> None:
    ...
    def test_mock_custom_fields(self, scraper: SymbolMarkets) -> None:
        ...
        with patch.object(scraper, "_request") as mock_req:
            ...
            result = scraper.get_symbol_markets(
                symbol="AAPL", scanner="america", fields=custom_fields
            )
            assert "status" in result

    def test_mock_empty_fields(self, scraper: SymbolMarkets) -> None:
        """Test with empty fields list."""
        mock_data: list = []
        with patch.object(scraper, "_request") as mock_req:
            mock_req.return_value = ({"data": mock_data, "totalCount": 0}, None)
            result = scraper.get_symbol_markets(
                symbol="AAPL", scanner="global", fields=[]
            )
            assert "status" in result
...
class TestMockSymbolMarketsLimits:
    ...
    def test_mock_limit_50(self, scraper: SymbolMarkets) -> None:
        """Test limit of 50."""
        mock_data = [
        ...
        with patch.object(scraper, "_request") as mock_req:
            ...
            result = scraper.get_symbol_markets(
                symbol="AAPL", scanner="global", limit=50
            )
            if result["status"] == STATUS_SUCCESS:
                assert len(result["data"]) <= 50

    def test_mock_limit_100(self, scraper: SymbolMarkets) -> None:
        """Test limit of 100."""
        mock_data = [
        ...
        with patch.object(scraper, "_request") as mock_req:
            ...
            result = scraper.get_symbol_markets(
                symbol="AAPL", scanner="global", limit=100
            )
            if result["status"] == STATUS_SUCCESS:
                assert len(result["data"]) <= 100

    def test_mock_limit_150(self, scraper: SymbolMarkets) -> None:
        """Test limit of 150."""
        mock_data = [
        ...
        with patch.object(scraper, "_request") as mock_req:
            ...
            result = scraper.get_symbol_markets(
                symbol="AAPL", scanner="global", limit=150
            )
            if result["status"] == STATUS_SUCCESS:
...
class TestMockSymbolMarketsValidation:
    ...
    def test_mock_exchange_symbol_format(self, scraper: SymbolMarkets) -> None:
        """Test EXCHANGE:SYMBOL format extracts symbol correctly."""
        mock_data = [
...
class TestMockSymbolMarketsErrors:
    ...
    def test_mock_network_error(self, scraper: SymbolMarkets) -> None:
    ...
    def test_mock_empty_response(self, scraper: SymbolMarkets) -> None:
        """Test empty response returns error."""
        with patch.object(scraper, "_request") as mock_req:
            mock_req.return_value = ({"data": []}, None)
            result = scraper.get_symbol_markets(symbol="NONEXISTENT", scanner="global")
            assert result["status"] == "failed"
            assert "No markets found" in result["error"]

    def test_mock_timeout_error(self, scraper: SymbolMarkets) -> None:
...
class TestMockSymbolMarketsScanners:
    ...
    def test_mock_all_scanners(self, scraper: SymbolMarkets, scanner: str) -> None:
        """Test all supported scanners."""
        mock_data = [
        ...
        with patch.object(scraper, "_request") as mock_req:
            mock_req.return_value = ({"data": mock_data, "totalCount": 1}, None)
            result = scraper.get_symbol_markets(
                symbol="SYMBOL", scanner=scanner, limit=10
            )
            assert "status" in result
...
class TestMockSymbolMarketsExport:
    ...
    def test_mock_export_enabled(self, scraper: SymbolMarkets) -> None:
    ...
    def test_mock_export_disabled(self, scraper: SymbolMarkets) -> None:
        ...
        with patch.object(scraper, "_request") as mock_req:
            mock_req.return_value = ({"data": mock_data, "totalCount": 1}, None)
            with patch.object(scraper, "_export") as mock_export:
                scraper.get_symbol_markets(symbol="AAPL", scanner="global")
...
class TestMockSymbolMarketsResponseEnvelope:
    ...
    def test_mock_success_envelope(self, scraper: SymbolMarkets) -> None:
    ...
    def test_mock_metadata_fields(self, scraper: SymbolMarkets) -> None:
        ...
        with patch.object(scraper, "_request") as mock_req:
            mock_req.return_value = ({"data": mock_data, "totalCount": 1}, None)
            result = scraper.get_symbol_markets(
                symbol="AAPL", scanner="america", limit=50
            )
            if result["status"] == STATUS_SUCCESS:
